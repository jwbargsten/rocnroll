  ...
  public:
    void testPRavg(void)
    {
      /* 
       library(ROCR)
       data(ROCR.xval)
       cv.name <- lapply(seq(along=ROCR.xval$predictions), function(x, sample) { rep(letters[x], length(sample[[x]]) ) }, ROCR.xval$predictions)
       d <- data.frame(cv=unlist(cv.name), p=unlist(ROCR.xval$predictions), l=unlist(ROCR.xval$labels))
        
       write.table(d, "rocr.xval.1-10cv.tsv", row.names=F, col.names=F, quote=F,sep="\t")
